...
class TeacherController extends _.BaseController {
  ...
  async login(req, res) {
    try {
      ...
      const teacher = await _teacher.default.findByCredentials(email, password);
      const token = await teacher.generateAuthToken();
      const body = {
        teacher,
        token
    ...
    if (req.user.approved !== true && req.user.status !== 'Approved') {
      ...
      try {
        const teachers = await _teacher.default.find({
          approved: true,
          $or: [{
            subjectOrClass: 'English Literature'
          }]
        });
        super.success(res, teachers || [], 'Successfully Retrieved all Teachers.');
      } catch (e) {
    ...
    if (req.user.approved !== true && req.user.status !== 'Approved') {
      ...
      try {
        const teachers = await _teacher.default.find({
          approved: true,
          $or: [{
            subjectOrClass: 'Islamic Religious Studies'
          }]
        });
        super.success(res, teachers || [], 'Successfully Retrieved all Teachers.');
      } catch (e) {
    ...
    if (req.user.approved !== true && req.user.status !== 'Approved') {
      ...
      try {
        const teachers = await _teacher.default.find({
          approved: true,
          $or: [{
            subjectOrClass: 'Civic Education'
          }]
        });
        super.success(res, teachers || [], 'Successfully Retrieved all Teachers.');
      } catch (e) {
